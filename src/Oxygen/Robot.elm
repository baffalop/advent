...
type alias Map =
    { droidLocation : ( Int, Int )
    , surroundings : Dict ( Int, Int ) Element
    }
...
decodeFeedback : Int -> Maybe Feedback
decodeFeedback code =
    case code of
        0 ->
            Just Blocked
...
print : Map -> String
print { droidLocation, surroundings } =
    let
        map =
            case Dict.get droidLocation surroundings of
                Just Oxygen ->
                    surroundings

                _ ->
                    Dict.insert droidLocation Droid surroundings
        ...
        printElement e =
            case e of
                ...
                Oxygen ->
                    'O'
    in
    "\n" ++ Printer.print printElement map ++ "\n"
    ...
    let
        ...
        feedbackResult =
            case output of
                ...
                [ x ] ->
                    Result.fromMaybe
                        ("Unrecognised feedback code " ++ String.fromInt x)
                        (decodeFeedback x)
...
updateMap : Map -> Direction -> Feedback -> Map
updateMap { droidLocation, surroundings } direction feedback =
    let
        moveCoord =
            applyDirection droidLocation direction
    ...
    , surroundings = Dict.insert moveCoord newElement surroundings
    }
...
navigate : List Direction -> String
navigate moves =
    List.foldl
        (\dir -> Result.andThen (move dir))
        (Ok init)
        moves
